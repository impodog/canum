...
#[require(Behavior::new("Apple_MoveCloser", 1.0, ["Displacement"]), BaseFartherBetter::new(150.0, 70.0))]
struct MoveCloser;
...
#[require(
    Behavior::new("Apple_ThrowSlice", 2.0, ["Projectile"]),
    BaseByDistance::new(200.0, 100.0)
)]
pub struct ThrowSlice;
fn throw_slice_start(
    event: On<BehaveStart>,
    mut commands: Commands,
    q_transform: Query<&GlobalTransform>,
    player: Res<player::RandomPlayer>,
    ...
    commands.trigger(BehaveEnd {
        entity: event.entity,
        // Cooldown exists in
        cooldown: Duration::from_secs_f32(rand_normal(2.0, 0.5)),
    });
    Ok(())
}
