...
/// The entity will be teleported inside bounds if outside.
#[derive(Component, Default)]
pub struct MoveMeBack;

#[derive(Resource, Default, Debug, Clone, Copy, Deref, DerefMut)]
/// Defines the boundaries of the whole arena.
pub struct ScreenBounds(pub Rect);

fn move_me_back(
    mut q_player: Query<(&mut Transform, &GlobalTransform), With<MoveMeBack>>,
